...
createCarryObject(ownerTeam,trigger,visuals,offset)
{
	...
	if(isSubStr(trigger.classname,"use"))carryObject.triggerType="use";
	else carryObject.triggerType="proximity";
	...
	if(carryObject.triggerType=="use")carryObject thread carryObjectUseThink();
	else carryObject thread carryObjectProxThink();
...
carryObjectUseThink()
{
	...
	for(;;)
	{
		...
		if(self.isResetting||!isAlive(player)||!self canInteractWith(player.pers["team"])||!player.canPickupObject||player.throwingGrenade||isDefined(self.carrier))continue;
...
carryObjectProxThink()
{
	...
	{
		self.trigger waittill("trigger",player);
		if(self.isResetting||!isAlive(player)||!self canInteractWith(player.pers["team"])||!player.canPickupObject||isDefined(self.carrier))continue;
		if((isDefined(level.timeout_over)&&!level.timeout_over)||(isDefined(game["PROMOD_KNIFEROUND"])&&game["PROMOD_KNIFEROUND"]))return;
		self setPickedUp(player);
	}
}
...
pickupObjectDelay(origin)
{
	...
	for(;;)
	{
		if(distanceSquared(self.origin,origin)>4096)break;
		wait 0.2;
	}
	self.canPickupObject=true;
}
...
setPickedUp(player)
{
	...
	self.carrier=player;
	for(i=0;i<self.visuals.size;i++)self.visuals[i]hide();
	self.trigger.origin+=(0,0,10000);
	self notify("pickup_object");
	if(isDefined(self.onPickup))self[[self.onPickup]](player);
	...
	for(;;)
	{
		if(isDefined(self.carrier))
		{
			...
			if((self.visibleTeam=="friendly"||self.visibleTeam=="any")&&self isFriendlyTeam("allies")&&self.objIDPingFriendly)
			{
				...
				{
					self.objPoints["allies"].alpha=self.objPoints["allies"].baseAlpha;self.objPoints["allies"]fadeOverTime(objPingDelay+1);
...
returnHome()
{
	...
	if(isDefined(self.onReset))self[[self.onReset]]();
	self clearCarrier();
	updateWorldIcons();
	updateCompassIcons();
...
setDropped()
{
	...
	if(isDefined(trace))
	{
		...
		else dropAngles=(0,tempAngle,0);
		for(i=0;i<self.visuals.size;i++)
		{
			self.visuals[i].origin=dropOrigin;self.visuals[i].angles=dropAngles;
			self.visuals[i]show();
		}
	...
	if(isDefined(self.onDrop))self[[self.onDrop]](droppingPlayer);
	self clearCarrier();
	self updateCompassIcons();
	self updateWorldIcons();
	self.isResetting=false;
...
clearCarrier()
{
	if(!isdefined(self.carrier))return;
...
pickupTimeout()
{
	...
	{
		if(!self.visuals[0]isTouching(mineTriggers[i]))continue;
		self returnHome();
		return;
	}
	for(i=0;i<hurtTriggers.size;i++)
	{
		if(!self.visuals[0]isTouching(hurtTriggers[i]))continue;
		self returnHome();
		return;
	}
	if(isDefined(self.autoResetTime))
	{
		...
		if(!isDefined(self.carrier))self returnHome();
	}
}

takeObject(object)
{
	if(isDefined(self.carryIcon))self.carryIcon destroyElem();
...
trackCarrier()
{
	...
	while(isDefined(self.carryObject)&&isAlive(self))
	{
		if(self isOnGround())
		{
			...
			if(trace["fraction"]<1)self.carryObject.safeOrigin=trace["position"];
		}
		wait 0.05;
	}
}
...
createUseObject(ownerTeam,trigger,visuals,offset)
{
	...
	useObject.entNum=trigger getEntityNumber();
	useObject.keyObject=undefined;
	if(isSubStr(trigger.classname,"use"))useObject.triggerType="use";
	else useObject.triggerType="proximity";
	useObject.trigger=trigger;
	for(i=0;i<visuals.size;i++)
	{
		visuals[i].baseOrigin=visuals[i].origin;visuals[i].baseAngles=visuals[i].angles;
	}
	useObject.visuals=visuals;if(!isDefined(offset))offset=(0,0,0);
	useObject.offset3d=offset;useObject.compassIcons=[];
	useObject.objIDAllies=getNextObjID();
	useObject.objIDAxis=getNextObjID();
	...
	if(useObject.triggerType=="proximity")
	{
		...
		useObject.touchList["allies"]=[];
		useObject.touchList["none"]=[];
		useObject.useRate=0;useObject.claimTeam="none";
		useObject.claimPlayer=undefined;
		useObject.lastClaimTeam="none";
		useObject.lastClaimTime=0;
		useObject thread useObjectProxThink();
	}
	else{useObject.useRate=1;
...
useObjectUseThink()
{
	if((isDefined(game["promod_do_readyup"])&&game["promod_do_readyup"])||(isDefined(game["PROMOD_MATCH_MODE"])&&game["PROMOD_MATCH_MODE"]=="strat"))return;
	level endon("game_ended");
	for(;;)
	{
		self.trigger waittill("trigger",player);
		while(isDefined(player)&&player isTouching(self.trigger)&&!player isOnGround())wait 0.05;
		if(!isAlive(player)||!self canInteractWith(player.pers["team"])||!player isTouching(self.trigger)||!player useButtonPressed())continue;
		if(isDefined(self.keyObject)&&(!isDefined(player.carryObject)||player.carryObject!=self.keyObject))
		{
			if(isDefined(self.onCantUse))self[[self.onCantUse]](player);
			continue;
		}
		result=true;
		if(self.useTime>0)
		{
			if(isDefined(self.onBeginUse))self[[self.onBeginUse]](player);
			team=player.pers["team"];
			result=self useHoldThink(player);
			if(isDefined(self.onEndUse)&&isDefined(player))self[[self.onEndUse]](team,player,result);
		}
		if(!result)continue;
		if(isDefined(self.onUse))self[[self.onUse]](player);
	}
}

getEarliestClaimPlayer()
{
	team=self.claimTeam;earliestPlayer=self.claimPlayer;
	if(self.touchList[team].size>0)
	{
		...
		for(i=0;i<players.size;i++)
		{
			...
			{
				earliestPlayer=touchdata.player;earliestTime=touchdata.starttime;
			}
		}
	}
...
useObjectProxThink()
{
	...
	for(;;)
	{
		...
		{
			self.curProgress=0;creditPlayer=getEarliestClaimPlayer();
			if(isDefined(self.onEndUse))self[[self.onEndUse]](self getClaimTeam(),creditPlayer,isDefined(creditPlayer));
			if(isDefined(creditPlayer)&&isDefined(self.onUse))self[[self.onUse]](creditPlayer);self setClaimTeam("none");
		...
		if(self.claimTeam!="none")
		{
			if(self.useTime)
			{
				...
				{
					if(isDefined(self.onEndUse))self[[self.onEndUse]](self getClaimTeam(),self.claimPlayer,false);
				...
				else
				{
					...
					if(isDefined(self.onUseUpdate))self[[self.onUseUpdate]](self getClaimTeam(),self.curProgress/self.useTime,(50*self.useRate)/self.useTime);
				}
			}
			else
			{
				if(isDefined(self.onUse))self[[self.onUse]](self.claimPlayer);
				self setClaimTeam("none");
				self.claimPlayer=undefined;
			}
		}
	...
	for(;;)
	{
		...
		if(self canInteractWith(player.pers["team"])&&self.claimTeam=="none")
		{
			if(!isDefined(self.keyObject)||(isDefined(player.carryObject)&&player.carryObject==self.keyObject))
			{
				...
				if(self.useTime&&isDefined(self.onBeginUse))self[[self.onBeginUse]](self.claimPlayer);
			}
			else
			{
				if(isDefined(self.onCantUse))self[[self.onCantUse]](player);
		...
		if(self.useTime&&isAlive(player)&&!isDefined(player.touchTriggers[entityNumber]))player thread triggerTouchThink(self);
	}
}

setClaimTeam(newTeam)
{
	if(self.claimTeam=="none"&&getTime()-self.lastClaimTime>1000)self.curProgress=0;
	else if(newTeam!="none"&&newTeam!=self.lastClaimTeam)self.curProgress=0;
...
triggerTouchThink(object)
{
	...
	object.numTouching[team]--;object updateUseRate();
}
...
updateProxBar(object,forceRemove)
{
	...
	{
		if(isDefined(self.proxBar))self.proxBar hideElem();
		if(isDefined(self.proxBarText))self.proxBarText hideElem();
		return;
	}
	if(!isDefined(self.proxBar))
	{
		...
		self.proxBar.alpha=1;self.proxBar.lastUseRate=-1;
	}
	...
	{
		if(object.curProgress>object.useTime)object.curProgress=object.useTime;
...
updateUseRate()
{
	...
	numOther=0;
	if(self.claimTeam!="axis")numOther+=self.numTouching["axis"];
	if(self.claimTeam!="allies")numOther+=self.numTouching["allies"];
	self.useRate=0;
	if(numClaimants&&!numOther)self.useRate=numClaimants;
}
...
useHoldThink(player)
{
	...
	if(isdefined(result)&&result)return true;
	if(isDefined(player))
	{
		player.claimTrigger=undefined;
		player _enableWeapon();
		if(!isAlive(player))player.killedInUse=true;
	}
	self.inUse=false;
...
updateTrigger()
{
	if(self.triggerType!="use")return;
	if(self.interactTeam=="none")self.trigger.origin-=(0,0,50000);
	else if(self.interactTeam=="any")
	{
		self.trigger.origin=self.curOrigin;self.trigger setTeamForTrigger("none");
	}
	else if(self.interactTeam=="friendly")
	{
		self.trigger.origin=self.curOrigin;
		if(self.ownerTeam=="allies")self.trigger setTeamForTrigger("allies");
		else if(self.ownerTeam=="axis")self.trigger setTeamForTrigger("axis");
		else self.trigger.origin-=(0,0,50000);
	}
	else if(self.interactTeam=="enemy")
	{
		self.trigger.origin=self.curOrigin;
		if(self.ownerTeam=="allies")self.trigger setTeamForTrigger("axis");
		else if(self.ownerTeam=="axis")self.trigger setTeamForTrigger("allies");
		else self.trigger setTeamForTrigger("none");
	}
}
...
{
	if(!isDefined(self.worldIcons[relativeTeam]))showIcon=false;
	...
	for(i=0;i<updateTeams.size;i++)
	{
		...
		if(showIcon)
		{
			...
			if(isDefined(self.compassIcons[relativeTeam]))objPoint setWayPoint(true,self.worldIcons[relativeTeam]);
			else objPoint setWayPoint(true);
...
updateCompassIcon(relativeTeam,showIcon)
{
	...
	for(i=0;i<updateTeams.size;i++)
	{
		...
		if(updateTeams[i]=="axis")objId=self.objIDAxis;
		...
		if(self.type=="carryObject")
		{
			if(isAlive(self.carrier)&&!shouldPingObject(relativeTeam))objective_onentity(objId,self.carrier);
			else objective_position(objId,self.curOrigin);
		}
	}
}
...
{
	if((relativeTeam=="friendly"&&self.objIDPingFriendly)||(relativeTeam=="enemy"&&self.objIDPingEnemy))return true;return false;
}
...
getUpdateTeams(relativeTeam)
{
	...
	{
		if(self isFriendlyTeam("allies"))updateTeams[0]="allies";
		else if(self isFriendlyTeam("axis"))updateTeams[0]="axis";
	}
	else if(relativeTeam=="enemy")
	{
		if(!self isFriendlyTeam("allies"))updateTeams[updateTeams.size]="allies";
		if(!self isFriendlyTeam("axis"))updateTeams[updateTeams.size]="axis";
	}
	return updateTeams;
}
...
{
	self.interactTeam=relativeTeam;updateTrigger();
}
...
setModelVisibility(visibility)
{
	if(visibility)
	{
		...
		{
			self.visuals[i]show();
			if(self.visuals[i].classname=="script_brushmodel"||self.visuals[i].classname=="script_model")self.visuals[i]thread makeSolid();
		}
	}
	else
	{
...
makeSolid()
{
	...
	for(;;)
	{
		...
		{
			if(level.players[i]isTouching(self))break;
		}
...
{
	if((isDefined(game["promod_do_readyup"])&&game["promod_do_readyup"])||(isDefined(game["PROMOD_MATCH_MODE"])&&game["PROMOD_MATCH_MODE"]=="strat"))return;
	self.compassIcons[relativeTeam]=shader;updateCompassIcons();
}

set3DIcon(relativeTeam,shader)
{
	if((isDefined(game["promod_do_readyup"])&&game["promod_do_readyup"])||(isDefined(game["PROMOD_MATCH_MODE"])&&game["PROMOD_MATCH_MODE"]=="strat"))return;
	self.worldIcons[relativeTeam]=shader;updateWorldIcons();
}
...
disableObject()
{
	...
	if(level.gametype=="sd")self setVisibleTeam(game["defenders"]);
	else self setVisibleTeam("none");
}
...
enableObject()
{
	if(self.type=="carryObject")
	{
		for(i=0;i<self.visuals.size;i++)self.visuals[i]show();
	}
...
{
	if(team==self.ownerTeam)return"friendly";
	else if(team==self.enemyTeam)return"enemy";
	else return"neutral";
}

isFriendlyTeam(team)
{
	if(self.ownerTeam=="any"||self.ownerTeam==team)return true;
	return false;
}
...
canInteractWith(team)
{
	switch(self.interactTeam)
	{
		...
		case"friendly":if(team==self.ownerTeam)return true;
		else return false;
		case"enemy":if(team!=self.ownerTeam)return true;
		else return false;
